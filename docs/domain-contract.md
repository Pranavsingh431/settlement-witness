...
`docs/schema/v5/` is generated from them. Where this page and the code ever
...
- `PaymentEvent` covers `CAPTURE`, `REFUND`, `REVERSAL` and `CHARGEBACK`. The
  amount must be strictly greater than zero, and the model refuses anything
  else; direction comes from the type. Every event names the source fact it was
  read from.

  Zero is refused because an event amount is the magnitude of something that
  happened, and zero is the absence of it rather than a smaller version. It also
  used to be exploitable: a zero refund made INV-009 not applicable, because a
  return existed, while firing no lifecycle code, because zero is neither a
  partial nor a full return. A settlement line settling any amount at all could
  then resolve.

  This constraint belongs to `PaymentEvent`, not to `Money`. A settlement net,
  an adjustment, a fee, a tax and a payout total may all validly be zero, and a
  net or an adjustment may be negative.
...
| INV-009 | Settlement gross equals the capture it settles | `INSUFFICIENT_EVIDENCE` | Yes |
...
INV-009 exists because the others leave a gap. INV-002 checks only that a line
is internally consistent and INV-003 checks only that a batch adds up, so a line
declaring it settled 80000 against a capture of 100000 satisfies both, and the
20000 difference appears nowhere in the result. An unexplained monetary
difference is the one thing a reconciliation system must never call resolved.

It applies to the shape a direct match supports: exactly one capture and nothing
returned. Anything else is `NOT_APPLICABLE`, because the relationship between a
settled gross and a capture stops being a plain equality once money has gone
back or once there is more than one capture to settle against. Those shapes
carry their own non-resolution codes, so nothing passes by being not applicable
here. A currency difference fails rather than converting: this layer has no
...
Each version so far has been a major step, and each removed wrong answers
rather than adding capability.

| Version | Change | Why breaking |
| --- | --- | --- |
| 2.0.0 | Status derived, evidence resolved against real facts | Decisions 1.0.0 accepted became unconstructible |
| 3.0.0 | `DecisionCandidate` lost `reason_codes`; the verifier derives them | A field was removed |
| 4.0.0 | INV-009 added and required for resolution | Decisions 3.0.0 resolved now need a further passing check |
| 5.0.0 | Payment event amounts must be strictly positive | Events 4.0.0 accepted are now refused at construction |

The schema directory follows the major version, and the superseded one is
removed rather than left in place. Each described a contract with a known
correctness gap, and nothing consumes any of them because decisions are still
not persisted. The general rule from ADR-002, that a superseded directory stays
intact, applies to any version that has actually been used.

The JSON Schema in `docs/schema/v5/` is generated by `make schema`. A test
